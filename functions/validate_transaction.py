def validate_transaction(transaction, network):
    ...
    if (
        "id" not in transaction
        or transaction["id"] is None
        or len(transaction["id"]) == 0
    ):
        return False
    if (
        "sender" not in transaction
        or transaction["sender"] is None
        or len(transaction["sender"]) == 0
    ):
        return False
    if (
        "receiver" not in transaction
        or transaction["receiver"] is None
        or len(transaction["receiver"]) == 0
    ):
        return False
